...
# start the compose file
  ...
  docker-compose -f config/testbed/unit-test.yml -p $module up -d $containers
  # let's see'um
  ...
  docker-compose -f config/testbed/unit-test.yml -p $module up -d utest
  ...
  echo " stop unit-test.yml services from provision-utests ...."
  docker-compose -p $module -f config/testbed/unit-test.yml stop
  sleep 10
  ...
  echo " remove unit-test.yml services from provision-utests ...."
  docker-compose -p $module -f config/testbed/unit-test.yml rm -f
  sleep 10
