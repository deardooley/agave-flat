...
# Do some work. In this case, tar up the source directory/file to a destination path unless it is already a tgz
shopt -s nocasematch
if [ ${sourcePath: -4} == ".tgz" ] || [ ${sourcePath: -7} == ".tar.gz" ]; then
    mv $sourcePath $destPath
else
    tar -zcf $destPath $sourcePath
fi
