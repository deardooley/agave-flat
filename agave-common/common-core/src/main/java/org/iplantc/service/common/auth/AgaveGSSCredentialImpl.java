 ...
 */
public class AgaveGSSCredentialImpl implements ExtendedGSSCredential,
		Serializable {
	 ...
	 *            credential usage
	 */
	public AgaveGSSCredentialImpl(AgaveX509Credential cred, int usage)
			throws GSSException {
		if (cred == null) {
...
//		this.trustedCALocation = cred.getTrustedCALocation();
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof AgaveGSSCredentialImpl) {
			AgaveGSSCredentialImpl other = (AgaveGSSCredentialImpl) obj;
			return other.usage == this.usage
					&& (other.cred == this.cred || this.cred != null
	...
	public int getRemainingInitLifetime(Oid mech) throws GSSException {
		GlobusGSSManagerImpl.checkMechanism(mech);
		if (this.usage == INITIATE_ONLY || this.usage == INITIATE_AND_ACCEPT) {
	...
	public int getRemainingAcceptLifetime(Oid mech) throws GSSException {
		GlobusGSSManagerImpl.checkMechanism(mech);
		if (this.usage == ACCEPT_ONLY || this.usage == INITIATE_AND_ACCEPT) {
			return getRemainingLifetime();
		} else {
			throw new GSSException(GSSException.FAILURE);
		}
	...
	public byte[] export(int option, Oid mech) throws GSSException {
		...
		if (this.cred == null) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.CREDENTIAL_ERROR, "anonCred00");
		}
	 ...
	 * certificate chain of this credential (<code>X509Certificate[]</code>).</LI>
	 * </UL>
	 *
	 * @param oid
	...
	/**
	 * Returns actual X509Credential object represented by this credential (if
	 * any).
	 *
	 * @return The credential object. Might be null if this is an anonymous
	 *         credential.
	 */
	 ...
	 *
	 * @return The private key. Might be null if this is an anonymous
	 *         credential.
	 */
	 ...
	 *
	 * @return The certificate chain. Might be null if this is an anonymous
	 *         credential.
	 */
	...
	public String getTrustrootPath() {
		return this.cred.getTrustedCALocation().getCaPath();
	}
