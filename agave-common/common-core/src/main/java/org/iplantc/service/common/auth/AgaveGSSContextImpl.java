...
public class AgaveGSSContextImpl implements ExtendedGSSContext {
	...
	// would be talking to old GRAM servers.
	private static final String[] GRAM_ENCRYPTION_CIPHER_SUITES = { "SSL_RSA_WITH_3DES_EDE_CBC_SHA" };
	private static final String[] GRAM_NO_ENCRYPTION_CIPHER_SUITES = { "SSL_RSA_WITH_NULL_SHA" };
	...
	/* handshake states */
	private static final int HANDSHAKE = 0, CLIENT_START_DEL = 2,
			CLIENT_END_DEL = 3, SERVER_START_DEL = 4, SERVER_END_DEL = 5;

	/** Delegation state */
	protected int delegationState = DELEGATION_START;

	/* delegation states */
	private static final int DELEGATION_START = 0, DELEGATION_SIGN_CERT = 1,
	...
	protected AgaveGSSCredentialImpl ctxCred;
	 ...
	 */
	public AgaveGSSContextImpl(GSSName target, AgaveGSSCredentialImpl cred)
			throws GSSException {
		...
		try {
			...
			if (System.getProperty("com.sun.net.ssl.rsaPreMasterSecretFix") == null)
				System.setProperty("com.sun.net.ssl.rsaPreMasterSecretFix",
						"true");

			// WARNING WARNING:
			// The new jglobus2-based srm-client is not compatible with old
			// bestman2
			// servers UNLESS we change this setting.
			//
			// The protection we are turning off helps against the BEAST attack.
			// When enabled, it will insert empty TLS application records into
			// the
			// stream. However, the old server will deadlock on the extra
			// records.
			//
			// To our knowledge, the BEAST attack is not applicable to this
			// client as
			// we don't have any concurrent insecure connections. Regardless, we
			// ought
			// to remove this as soon as we can drop support for the old
			// servers.
			//
			// -BB. Sept 24, 2012.
			//
	...
	public byte[] acceptSecContext(byte[] inBuff, int off, int len)
		...
		if (savedInBytes != null) {
			if (len > 0) {
				...
				logger.debug("ALLOCATED for allInBytes " + savedInBytes.length
						+ " + " + len + " bytes\n");
				System.arraycopy(savedInBytes, 0, allInBytes, 0,
						savedInBytes.length);
				System.arraycopy(inBuff, off, allInBytes, savedInBytes.length,
						len);
				inByteBuff = ByteBuffer.wrap(allInBytes, 0, allInBytes.length);
			} else {
				inByteBuff = ByteBuffer.wrap(savedInBytes, 0,
						savedInBytes.length);
			}
			savedInBytes = null;
		} else {
		...
		case HANDSHAKE:
			...
			try {
				...
				SSLEngineResult.HandshakeStatus handshake_status = sslEngine
						.getHandshakeStatus();
				...
				if (handshake_status == SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
					...
					outByteBuff = this.sslProcessHandshake(inByteBuff,
							outByteBuff);
				}
				...
				if (this.sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
					...
					// acceptor
					for (X509Certificate cert : this.ctxCred
							.getCertificateChain()) {
					...
					Certificate[] chain;
					try {
						chain = this.sslEngine.getSession()
								.getPeerCertificates();
					...
					if (chain == null || chain.length == 0) {
						...
						String identity = BouncyCastleUtil
								.getIdentity(bcConvert(BouncyCastleUtil
										.getIdentityCertificate((X509Certificate[]) chain)));
						this.sourceName = new GlobusGSSName(
								CertificateUtil.toGlobusID(identity, false));
						this.peerLimited = Boolean
								.valueOf(ProxyCertificateUtil.isLimitedProxy(BouncyCastleUtil
			...
			try {
				...
				if (chain == null || chain.length == 0) {
					throw new GlobusGSSException(GSSException.FAILURE,
							GlobusGSSException.DELEGATION_ERROR, "noClientCert");
				}
		...
		case SERVER_END_DEL:
			...
			try {
				 ...
				 * DEL Vector chain = this.conn.getCertificateChain();
				 */
				Certificate[] chain = this.sslEngine.getSession()
						.getPeerCertificates();
				...
				AgaveX509Credential proxy = new AgaveX509Credential(
						this.keyPair.getPrivate(), newChain);

				this.delegCred = new AgaveGSSCredentialImpl(proxy,
		...
		if (inByteBuff.hasRemaining()) {
			...
			logger.debug("SAVING unprocessed " + inByteBuff.remaining()
					+ "BYTES\n");
		...
		try {
			...
			do {
				...
				if (result.getStatus() == SSLEngineResult.Status.BUFFER_OVERFLOW) {
					...
					ByteBuffer b = ByteBuffer.allocate(pktSize
							+ outBBuff.position());
					outBBuff.flip();
					b.put(outBBuff);
					outBBuff = b;
				...
				if (result.getStatus() != SSLEngineResult.Status.OK) {
					throw new GlobusGSSException(GSSException.FAILURE,
							GlobusGSSException.TOKEN_FAIL, result.getStatus()
									.toString());
				}
		...
		try {
			...
			do {
				...
				if (result.getStatus() == SSLEngineResult.Status.BUFFER_OVERFLOW) {
					...
					int appSize = sslEngine.getSession()
	...
	private ByteBuffer sslProcessHandshake(ByteBuffer inBBuff,
			ByteBuffer outBBuff) throws GSSException {
		...
		try {
				...
				while (sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NEED_WRAP) {
					...
					if (result.getStatus() == SSLEngineResult.Status.BUFFER_OVERFLOW) {
						// increase it to the size needed.
						int pktSize = sslEngine.getSession()
								.getPacketBufferSize();
						ByteBuffer b = ByteBuffer.allocate(pktSize
								+ outBBuff.position());
						outBBuff.flip();
					...
					if (result.getStatus() != SSLEngineResult.Status.OK) {
						throw new GlobusGSSException(GSSException.FAILURE,
								GlobusGSSException.TOKEN_FAIL, result
										.getStatus().toString());
					}
				}
	...
	public byte[] initSecContext(byte[] inBuff, int off, int len)
		...
		if (!this.conn) {
			...
			if (getCredDelegState()) {
				if (this.gssMode == GSIConstants.MODE_SSL) {
					throw new GlobusGSSException(GSSException.FAILURE,
							GlobusGSSException.BAD_ARGUMENT, "initCtx00");
				}
				if (this.anonymity) {
					throw new GlobusGSSException(GSSException.FAILURE,
							GlobusGSSException.BAD_ARGUMENT, "initCtx01");
				}
			}
		...
		if (!Boolean.FALSE.equals(this.requireAuthzWithDelegation)) {
			...
			if (this.expectedTargetName == null && getCredDelegState()) {
				throw new GlobusGSSException(GSSException.FAILURE,
						GlobusGSSException.BAD_ARGUMENT, "initCtx02");
			}
		}
			...
			try {
				...
				if (this.sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
					...
					String identity = BouncyCastleUtil
							.getIdentity(bcConvert(BouncyCastleUtil
									.getIdentityCertificate((X509Certificate[]) chain)));
					this.targetName = new GlobusGSSName(
							CertificateUtil.toGlobusID(identity, false));
					...
					if (this.expectedTargetName != null
						...
						throw new GlobusGSSException(GSSException.UNAUTHORIZED,
								GlobusGSSException.BAD_NAME, "authFailed00",
		...
		case CLIENT_START_DEL:
			...
			if (this.state != CLIENT_START_DEL
					|| this.outByteBuff.remaining() > 0) {
				throw new GSSException(GSSException.FAILURE);
			}
			if (inByteBuff.hasRemaining()) {
				throw new GlobusGSSException(GSSException.FAILURE,
						new Exception("Not all data processed; Original: "
								+ len + " Remaining: " + inByteBuff.remaining()
								+ " Handshaking status: "
								+ sslEngine.getHandshakeStatus()));
			}
			this.outByteBuff.clear();
		...
		case CLIENT_END_DEL:
			...
			try {
				...
				X509Certificate cert = this.certFactory.createCertificate(
						byteArrayInputStream, chain[0], this.ctxCred
								.getPrivateKey(), -1,
						/*
		...
		// if (this.outByteBuff.hasRemaining() || this.state ==
		// CLIENT_START_DEL) {
		if (this.outByteBuff.hasRemaining()) {
			// TODO can we avoid this copy if the ByteBuffer is array based
			// and we return that array, each time allocating a new array
	...
	private void init(int how) throws GSSException, SSLException {
		...
		logger.debug("CIPHER SUITE IS: "
				+ Arrays.toString(this.sslEngine.getEnabledCipherSuites()));
	...
	/**
	 * Wraps a message for integrity and protection. A regular SSL-wrapped token
	 * is returned.
	 */
	...
	public byte[] unwrap(byte[] inBuf, int off, int len, MessageProp prop)
		 ...
		 * using get_mic
		 */
		if (inBuf[off] == GSI_WRAP && inBuf[off + 1] == 3
				&& inBuf[off + 2] == 0) {
			throw new GSSException(GSSException.UNAVAILABLE);
			/*
			 * DEL
	...
	private byte[] unwrap(byte[] inBuf, int off, int len) throws GSSException {
		...
		if (savedInBytes != null) {
			if (len > 0) {
				...
				System.arraycopy(savedInBytes, 0, allInBytes, 0,
						savedInBytes.length);
				System.arraycopy(inBuf, off, allInBytes, savedInBytes.length,
						len);
				inByteBuff = ByteBuffer.wrap(allInBytes, 0, allInBytes.length);
			} else {
				inByteBuff = ByteBuffer.wrap(savedInBytes, 0,
	...
	public void requestLifetime(int lifetime) throws GSSException {
		if (lifetime == GSSContext.INDEFINITE_LIFETIME) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.UNKNOWN, "badLifetime00");
		}
	...
	public int getLifetime() {
		if (this.goodUntil != null) {
			return (int) ((this.goodUntil.getTime() - System
	...
	public void requestInteg(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION, "integOn");
		}
	}
	...
	public void requestSequenceDet(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION, "seqDet");
		}
	}
	...
	public void requestReplayDet(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION, "replayDet");
		}
	}
	...
	public void requestMutualAuth(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION, "mutualAuthOn");
		}
	}
	...
	protected byte[] generateCertRequest(X509Certificate cert)
		...
		int bits = ((RSAPublicKey) cert.getPublicKey()).getModulus()
				.bitLength();
	...
	protected void setGssMode(Object value) throws GSSException {
		if (!(value instanceof Integer)) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION_TYPE, "badType",
		...
		if (v == GSIConstants.MODE_GSI || v == GSIConstants.MODE_SSL) {
			...
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION, "badGssMode");
		}
	}
	...
	protected void setDelegationType(Object value) throws GSSException {
		...
		if (v == GSIConstants.DelegationType.FULL
			...
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION, "badDelegType");
		}
	}
	...
	protected void setForceSslV3AndConstrainCipherSuitesForGram(Object value)
			throws GSSException {
		if (!(value instanceof Boolean)) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION_TYPE, "badType",
					new Object[] { "adjust cipher suites for GRAM",
							Boolean.class });
		}
	...
	protected void setProxyPolicyHandlers(Object value) throws GSSException {
		if (!(value instanceof Map)) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_OPTION_TYPE, "badType",
					new Object[] { "Proxy policy handlers", Map.class });
		}
	...
	public void setOption(Oid option, Object value) throws GSSException {
		if (option == null) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_ARGUMENT, "nullOption");
		}
		if (value == null) {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.BAD_ARGUMENT, "nullOptionValue");
		}
		...
		if (option.equals(GSSConstants.GSS_MODE)) {
			...
			setRequireAuthzWithDelegation(value);
				...
				.equals(GSSConstants.FORCE_SSLV3_AND_CONSTRAIN_CIPHERSUITES_FOR_GRAM)) {
			setForceSslV3AndConstrainCipherSuitesForGram(value);
		} else {
			throw new GlobusGSSException(GSSException.FAILURE,
					GlobusGSSException.UNKNOWN_OPTION, "unknownOption",
					new Object[] { option });
		}
	}
	...
	public byte[] initDelegation(GSSCredential credential, Oid mechanism,
			int lifetime, byte[] buf, int off, int len) throws GSSException {
		...
		case DELEGATION_SIGN_CERT:
			...
			X509Credential cred = ((AgaveGSSCredentialImpl) credential)
					.getX509Credential();

			X509Certificate[] chain = cred.getCertificateChain();

			int time = (lifetime == GSSCredential.DEFAULT_LIFETIME) ? -1
					: lifetime;
			...
			try {
				...
				X509Certificate cert = this.certFactory.createCertificate(
						inData, chain[0], cred.getPrivateKey(), time,
						/*
						 * DEL getDelegationType(chain[0]));
						 */
						BouncyCastleCertProcessingFactory.decideProxyType(
	 ...
	 *                <code>GSSException.FAILURE</code>
	 */
	public byte[] acceptDelegation(int lifetime, byte[] buf, int off, int len)
			throws GSSException {
		...
		case DELEGATION_START:
			...
			if (len != 1 && buf[off] != GSIConstants.DELEGATION_CHAR) {
				throw new GlobusGSSException(GSSException.FAILURE,
						GlobusGSSException.DELEGATION_ERROR, "delegError00",
			...
			try {
				...
				try {
					certChain = this.sslEngine.getSession()
							.getPeerCertificates();
				} catch (SSLPeerUnverifiedException e) {
					certChain = null;
				}
		...
		case DELEGATION_COMPLETE_CRED:
			...
			AgaveX509Credential proxy = new AgaveX509Credential(
					this.keyPair.getPrivate(), chain);

			this.delegatedCred = new AgaveGSSCredentialImpl(proxy,
					GSSCredential.INITIATE_AND_ACCEPT);
		...
		if (oid.equals(GSSConstants.X509_CERT_CHAIN)) {
			if (isEstablished()) {
				...
				try {
					...
					try {
						peerCerts = this.sslEngine.getSession()
								.getPeerCertificates();
					} catch (SSLPeerUnverifiedException e) {
						peerCerts = null;
					}
