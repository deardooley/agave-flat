        ...
        try
        {
            ...
            if (tr == null) {
            	throw new ResourceException(Status.CLIENT_ERROR_FORBIDDEN,
                        "No resource found for the given tag with uuid " + associatedUuid);
            } else {
            	ObjectMapper mapper = new ObjectMapper();
            
            	return Response.ok(new AgaveSuccessRepresentation(mapper.writeValueAsString(tr))).build();
            }
        }
        ...
        try
        {
            ...
            if (!pm.canWrite(getAuthenticatedUsername())) {
                ...
                try
                {
                    ...
                    TaggedResource tr = updatedTag.getTaggedResource(associatedUuid);
                    if (tr == null) {
                    	throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
	...
	public Response remove(@PathParam("entityId") String entityId,
        ...
        try
        {
            ...
            if (!pm.canWrite(getAuthenticatedUsername())) {
            	...
            	TagManager manager = new TagManager();
                manager.deleteUserTagAssociatedId(tag, associatedUuid, getAuthenticatedUsername());
            }
        	...
        	log.error(e);
        	throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
                    "Failed to remove "+associatedUuid+" from tag " + entityId + 
