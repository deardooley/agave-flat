...
public class JobSubmissionListener implements JobListener{
	
	private static final Logger log = Logger
			.getLogger(JobSubmissionListener.class);
	
    @Override
    public String getName() {
        return "Job submission attempt listener";
    }
    ...
    public void jobWasExecuted(JobExecutionContext context, JobExecutionException e) {
    	if (e == null) {
    		...
    		if (result instanceof JobStatusType) {
	    		...
	    		if (status == JobStatusType.QUEUED) {
	    			log.debug("Successfully completed submitting job " + jobUuid);
	    		} 
	    		else {
	    			...
	    			try {
	    				...
	    				log.info(String.format("Failed to submit job %s. "
	    				...
	    				log.info(String.format("Failed to submit job %s. "
	    						+ "No retry will be attempted according to the submission retry policy for this job", 
		    					jobUuid));
	    			}
	    		}
    		} else {
    			log.error("Unexpected result returned from job " + 
    					jobUuid + " submission attempt " + attempt);
    		}
        } else {
        	log.error("Unexpected termination of job submission attempt", e);
        }
    }
