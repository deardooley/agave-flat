...
public class ArchiveMessagingWatch extends AbstractJobWatch 
{
    private static final Logger log = Logger.getLogger(ArchiveMessagingWatch.class);
    
	public ArchiveMessagingWatch() {}
     ...
     */
    public String selectNextAvailableJobMessage() throws MessageProcessingException {
...
//                org.iplantc.service.common.Settings.getDedicatedSystemIdsFromServiceProperties());
        ...
        try
        {
            ...
            throw new MessageProcessingException("Failed to retrieve job archive message from queue", e);
        }
//        catch (Throwable e) {
...
//    }
     ...
     */
    public String processMessage(String body) throws MessageProcessingException
    {
        try 
        {
            ...
            if (messageBody == null || StringUtils.isEmpty(messageBody.getUuid())) {
                throw new MessageProcessingException("Job archive message had not body content.");
            } else {
        			...
        			while (!archived && attempts <= Settings.MAX_SUBMISSION_RETRIES)
        			{
        				...
        				catch (SystemUnavailableException e) 
						{
							try
							{
								...
								log.error("Failed to update job " + getJob().getUuid() + " status to PENDING");
							}	
							break;
						}
						catch (JobException e) 
