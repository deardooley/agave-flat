 ...
 extends FSM<T>
{
    ...
    /* ********************************************************************** */
    // Tracing.
    private static final Logger _log = Logger.getLogger(JobFSM.class);
     ...
     throws IllegalStateException
    {
        ...
        if (transition != null) {
            ...
            try {current = this.transition(stateful, current, event, transition, args);}
                 ...
                 String msg = "An attempt to retry an FSM transition occurred!";
            ...
            if (_strict) 
            {
                String msg = "No transition exists from state " +
                             current.getName() + " on event " +
                             event + ".";
