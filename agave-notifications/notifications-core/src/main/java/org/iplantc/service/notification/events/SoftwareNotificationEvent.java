                ...
                try {
                    ...
                    if (json.isObject()) {
                        if (json.has("job") ) {
                            ...
                            body = StringUtils.replace(body, "${APP_ID}", jsonJob.has("appId") ? jsonJob.get("appId").asText() : "");
                            body = StringUtils.replace(body, "${JOB_ID}", jsonJob.has("id") ? jsonJob.get("id").asText() : "");
                            body = StringUtils.replace(body, "${JOB_SYSTEM}", jsonJob.has("executionSystem") ? jsonJob.get("executionSystem").asText() : "");
                            body = StringUtils.replace(body, "${JOB_STATUS}", jsonJob.has("status") ? jsonJob.get("status").asText() : "");
                            body = StringUtils.replace(body, "${JOB_NAME}", jsonJob.has("name") ? jsonJob.get("name").asText() : "");
                            body = StringUtils.replace(body, "${JOB_OWNER}", jsonJob.has("owner") ? jsonJob.get("owner").asText() : "");
                            ...
                            body = StringUtils.replace(body, "${JOB_SUBMIT_TIME}", jsonJob.has("submitTime") ? new DateTime(jsonJob.get("submitTime").asText()).toString(): "");
                            body = StringUtils.replace(body, "${JOB_START_TIME}", jsonJob.has("startTime") ? new DateTime(jsonJob.get("startTime").asText()).toString(): "");
                            body = StringUtils.replace(body, "${JOB_END_TIME}", jsonJob.has("endTime") ? new DateTime(jsonJob.get("endTime").asText()).toString(): "");
                            body = StringUtils.replace(body, "${JOB_ARCHIVE_URL}", TenancyHelper.resolveURLToCurrentTenant(Settings.IPLANT_JOB_SERVICE) + jsonJob.get("id").asText() + "/outputs/listings");
                            body = StringUtils.replace(body, "${JOB_ARCHIVE_PATH}", jsonJob.has("appId") ? jsonJob.get("appId").asText() : "");
                            body = StringUtils.replace(body, "${JOB_URL}", TenancyHelper.resolveURLToCurrentTenant(Settings.IPLANT_JOB_SERVICE) + jsonJob.get("id").asText());
